...
func (c *StateClient) Set(ctx context.Context, taskID string, searchKey string, totalSources int) error {
	...
	c.logger.Info("Set task completed")
	return nil
}
...
func (c *StateClient) SetStatus(ctx context.Context, taskID string, status string) error {
	req := &taskstate.SetStatusRequest{
		TaskId: taskID,
		Status: status,
	}
	_, err := c.client.SetStatus(ctx, req)
	if err != nil {
		c.logger.Error(err.Error())
		return err
	}
...
func (c *StateClient) GetTotal(ctx context.Context, taskID string) (int64, error) {
	req := &taskstate.GetTotalRequest{
		TaskId: taskID,
	}
	resp, err := c.client.GetTotal(ctx, req)
	if err != nil {
		c.logger.Error(err.Error())
		return 0, err
	}
